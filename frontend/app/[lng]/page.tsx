...
const BasicInfoBar = async ({lng}: { lng: string }) => {
    ...
    return (
        <div className="mb-6">
            <div className="flex relative w-full min-h-[10rem] bg-title-bar">
                <div className="absolute w-40 h-40  translate-y-1/2 left-[3.5rem]">
                    <Image src={avatarImage} alt="" className="rounded-full border-4 border-white"/>
                </div>
                <div
                    className="text-white ml-[16.25rem] mr-[3.75rem] mb-5 flex flex-row justify-between w-full items-end">
                    <div>
                        <h1 className="text-4xl">{t("name")}</h1>
                    </div>
                    <div
                        className="flex flex-col gap-y-2 place-content-end">
            ...
            <div className="items-center mt-4 ml-[16.25rem] min-h-[3rem]">
                <div
                    className="shrink-0 w-fit text-[1.5rem] font-medium leading-normal text-main-text">
                    {t("jobTitle")}
                </div>
            </div>
        </div>
...
const FormatedMarkdown = ({markdown}: { markdown: string }) => {
    return (
        <MyMarkdown
            className="text-darkest-gray flex flex-col leading-1 [&_li]:list-disc [&_ul]:pl-6 [&_h3]:font-bold [&_ul]:mb-4 [&_p]:mb-4"
            markdown={markdown}/>
    );
}
        ...
        <div className="bg-main-page-background w-full flex flex-col items-center">
            <div className="mt-6 mb-8 w-full max-w-[980px] border-[1px] border-outline-gray bg-white">
                ...
                <div className="flex flex-col gap-10 px-10">
                    ...
                    <section className="px-5 py-4 flex-grow overflow-hidden">
                        ...
                        <div className="flex flex-col gap-8">
                            {
                                ...
                                t("job_history", {returnObjects: true}).map((job, index) => {
                                    return <section className="flex" key={index}>
                                        <div
                                            className="shrink-0 mt-1.5 mr-4 w-[180px] text-right text-middle-grey-2">
                                            {job.time}
                                        </div>
                                        <Pole/>
                                        <div className="grow ml-6">
                                            <div
                                                className="flex flex-row flex-wrap gap-y-0.5 justify-between items-start mb-3 font-bold text-main-text">
